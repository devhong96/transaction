...
@SpringBootTest
class SelfInvocationPizzaServiceTest {

    @Autowired
    private SelfInvocationPizzaService pizzaService;
    // ^ 여기 주입되는 객체가 바로 작성하신 'PizzaServiceProxy' (CGLIB 프록시)입니다.

    @Test
    @DisplayName("외부에서 getParent 호출 -> 내부에서 parent 호출 (Self-Invocation) -> 트랜잭션 적용 X")
    void testEatPizza_InternalCall() {

        // 프록시의 getParent()를 호출하지만, 내부에서 this.parent()를 부르므로 트랜잭션 코드를 건너뜀
        boolean isTxActive = pizzaService.eatPizza();
    ...
    void testPizza_DirectCall() {

        // 프록시의 parent()를 직접 호출하므로 txManager.begin()이 실행됨
        boolean isTxActive = pizzaService.pizza();
